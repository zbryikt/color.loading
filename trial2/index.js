...
x$.controller('main', ['$scope'].concat(function($scope){
  var r1, r2, delta, point, res$, i$, to$, ridx$, rad, sep, tick, color, ringBuilder, r2l, configs, hue, sat, lit, updatePtr, update, preWhich, target;
  r1 = 200;
  r2 = 170;
  delta = 2;
  res$ = [];
  ...
  updatePtr = function(){
    d3.select('#hue.ptr').attr({
      r: 8,
      cx: function(){
        return 190 * Math.cos(rad(hue));
      },
      cy: function(){
        return 190 * Math.sin(rad(hue));
      },
      fill: 'none',
      stroke: '#444',
      "stroke-width": '3px'
    ...
    d3.select('#sat.ptr').attr({
      r: 8,
      cx: function(){
        return 160 * Math.cos(rad(sat + hue));
      },
      cy: function(){
        return 160 * Math.sin(rad(sat + hue));
      },
      fill: 'none',
      stroke: '#444',
      "stroke-width": '3px'
    ...
    return d3.select('#lit.ptr').attr({
      r: 8,
      cx: function(){
        return 130 * Math.cos(rad(lit + hue - 90));
      },
      cy: function(){
        return 130 * Math.sin(rad(lit + hue - 90));
      },
      fill: 'none',
      stroke: '#444',
      "stroke-width": '3px'
